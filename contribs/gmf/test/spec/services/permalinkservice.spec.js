...
describe('Permalink service', function() {
  ...
  beforeEach(inject(function($injector) {
    ...
    let map = new ol.Map({layers: [], view: new ol.View({projection: ol.proj.get('EPSG:2056')})});
    ...
    let themesClone = goog.object.unsafeClone(themes);
    PermalinkService.themes_ = themesClone['themes'];


    //create fake layerTree
    let LayerHelper = $injector.get('ngeoLayerHelper');

    let dataGroup = LayerHelper.getGroupFromMap(map, gmf.DATALAYERGROUP_NAME);
    let firstLevelGroup = LayerHelper.createBasicGroup(new ol.Collection([
      LayerHelper.createBasicWMSLayer('', 'l_g1_1'),
      LayerHelper.createBasicWMSLayer('', 'l_g1_2')
    ]));

    let secondLevelGroup = LayerHelper.createBasicGroup(new ol.Collection([
  ...
  describe('#getMapCenter', function() {
    ...
    it('accepts flipped coordinates (x/y switched)', function() {
      ...
      let center = PermalinkService.getMapCenter();
