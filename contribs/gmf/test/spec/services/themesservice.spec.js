...
describe('gmf.Themes', function() {
  ...
  it('Get background layers', function() {
    ...
    themes.background_layers.forEach(function(bgLayer) {
      let response = bgLayer.name == 'map' ? capabilities.map :
          capabilities.asitvd;
    ...
    let response = spy.calls.mostRecent().args[0];
    expect(response.length).toBe(4);
    let responseFirstBgName = response[1].get('label');
    let firstBgName = themes.background_layers[0].name;
  ...
  it('Returns hasEditableLayers', function() {
    ...
    expect(spy.calls.count()).toBe(1);
    let response = spy.calls.mostRecent().args;
  ...
  it('Load themes', function() {
    ...
    expect(spy.calls.count()).toBe(1);
    let data = spy.calls.mostRecent().args[0];
  ...
  it('Get themes object', function() {
    ...
    expect(spy.calls.count()).toBe(1);
    let resultThemes = spy.calls.mostRecent().args[0];
    let dataFirstKey = Object.keys(resultThemes[0])[0];
    let themesThemesFirstKey = Object.keys(themes.themes[0])[0];
    expect(dataFirstKey).toBe(themesThemesFirstKey);
  });

  it('Get a theme object (find a specific theme)', function() {
    let themeName = 'Enseignement';
    let spy = jasmine.createSpy();
    ...
    expect(spy.calls.count()).toBe(1);
    let resultTheme = spy.calls.mostRecent().args[0];
